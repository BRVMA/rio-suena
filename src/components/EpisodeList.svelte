    ...
    onMount(() => {
        function setListHeight() {
            if (window.innerWidth < 640) {
                const usedHeight =
                    document.body.offsetHeight - listContainer.offsetHeight;
...
<div
    class={"flex flex-col lg:mr-16 2xl:mr-32 sm:mb-4 px-6 py-4 w-full bg-black bg-opacity-40 rounded-3xl " +
        (open ? "lg:max-w-lg" : "sm:max-w-3xl bg-transparent")}
>
    <div class={"flex justify-between " + (open ? "" : "sm:justify-end")}>
        ...
        <button class="hidden xl:block h-6" on:click={() => (open = !open)}>
            {#if open}
                <XSVG clase="w-6" />
            {:else}
                <MenuSVG clase="w-8" />
    ...
    <div class="flex-grow mt-2" bind:this={listContainer}>
        <ul
            class={"gap-4 mt-2 p-2 space-y-4 overflow-auto " +
                (open ? "" : "sm:flex sm:space-y-0")}
            bind:this={list}
        >
            {#each episodes as episode}
                <li>
                    <a
                        ...
                        class={"grid grid-cols-[3fr_4fr] flex-col gap-2 " +
                            (open ? "" : "sm:flex")}
                    >
                        <div
                        ...
                        <div>
                            <h4 class="mb-1 text-sm uppercase leading-tight">
                                <span class="font-light">{episode.title}</span
                                ><br />
                                <span class="font-bold">{episode.artist}</span>
                            </h4>

                            <p class={"text-xs " + (open ? "" : "sm:hidden")}>
                                {episode.description}
                            </p>
                        </div>
                    </a>
